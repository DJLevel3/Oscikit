...
def  main():
  ...
  installer = "\installer\Oscikit Installer.exe"
   
  if demo:
    installer = "\installer\Oscikit Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\Oscikit manual.pdf" 
  ]
  ...
  ZIPNAME = "Oscikit-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "Oscikit-v" + FULLVERSIONSTR + "-win-demo.zip"
